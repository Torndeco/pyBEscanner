...
def scan(logfile, log_offset):
    if os.path.isfile(logfile) is True:
        with open(logfile, 'r', encoding="utf-8", errors="replace") as f_log:
            f_log.seek(0,2)
...
class RPTScanner:
    ...
    def scan_log(self, lastline=1):
        if os.path.isfile(self.logfile) is False:
            ...
            if self.entries != []:
                with open(self.backup_rptlog, "a", encoding='utf-8') as rptlog_backup:
                    ...
                    while x < (len(self.entries) - lastline):
                        ...
                        x = x + 1
                self.offset_data["Lastline"] = self.entries[x]
                self.entries = []
...
class ConsoleScanner:
    ...
    def scan_log(self, lastline=1):
        if os.path.isfile(self.logfile) is False:
            ...
            self.offset_data["Offset"] = f_offset
            if self.entries != []:
                with open(self.backup_chatlog, "a", encoding='utf-8') as chatlog_backup, open(self.backup_consolelog, "a", encoding='utf-8') as consolelog_backup:
                    self.entries[0] = self.offset_data["Lastline"] + self.entries[0]
                    x = 0
                    while x < (len(self.entries) - lastline):
