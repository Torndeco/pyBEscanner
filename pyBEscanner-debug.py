...
class Main:
	...
	def loadconfig(self):
		...
		default["Bans"] = None # Doesnt initializes bans, until encounters a server with symlink setting turned on
		...
		for individual_option in options:
			default[individual_option[1]] = config.get("Default", individual_option[x][0])
		...
		for section in config_sections:
			...
			for individual_option in options:
				if config.has_option(section, individual_option[0]):
					server[individual_option[1]] = config.get(section, individual_option[0])
			...
			if server["Bans Symlinked"] == "on":
				print "Bans Symlinked = on"
				if default["Bans"] == None:
					print "Initialize Bans"
					default["Bans"] = bans.Bans(default["Bans Directory"])
				print "Server Bans = Default Bans"
				server["Bans"] = default["Bans"]
			else:
				print "Bans Symlinked = off"
				print "Initialize Bans"
				print "Server Bans = Server Bans"
				server["Bans"] = bans.Bans(server["BattlEye Directory"])
			print "-------"
			print str(server["Bans"])
			print "-------"
